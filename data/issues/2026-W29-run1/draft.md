*Week of 24 June - 15 July 2026*

Governance now leads this week's AI-in-finance news: Singapore's MAS set runtime safeguards for agentic finance, AI investment held steady near $202M, and liability questions sharpened.

---

## 1. AI in Finance

Singapore's MAS proposed AI agent safeguards for finance

**MAS** released **SAFR**, a runtime safeguard framework requiring AI agents in finance to operate within defined risk limits and mandates.

Source: https://dig.watch/updates/singapore-governance-framework-ai-agents-finance

Gartner predicts finance teams to go all-in on AI talent

**Gartner** predicts that by **2028**, one in five finance organizations will redirect all talent investment toward AI and digital skills, sidelining traditional hires.

Source: https://www.gartner.com/en/newsroom/press-releases/2026-07-01-gartner-predicts-20-percent-of-finance-orgs-will-pivot

AI investment in finance holds steady, KPMG finds

**KPMG's** Q2 survey found finance and banking teams plan **$202 million** in AI investment over 12 months, with only **26%** having real-time cost visibility.

Source: https://kpmg.com/us/en/media/news/q2-ai-pulse-2026.html

FactSet partnered with Google Cloud on AI

**FactSet** partnered with **Google Cloud** to embed AI models into its financial data platform, speeding research and analysis workflows for institutional clients.

Source: https://news.google.

Who's liable when AI gets a financial model wrong?

**CFO.com** highlighted a chance for finance teams to get ahead by setting clear accountability rules for **AI-assisted** financial models before problems arise.

Source: https://news.google.
